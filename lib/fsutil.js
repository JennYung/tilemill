...
var mkdir_p = require('../node_modules/npm/lib/utils/mkdir-p.js');
...
function _mkdirp(p, mode, f) {
    ...
    path.exists(p, function(exists) {
        ...
        mkdirp(ps.slice(0, -1).join(path_sep), mode, function(err) {
...
function mkdirpSync(p, mode) {
    ...
    while (ps.length) {
		...
		console.log('created: ' + created.join(path_sep));
        if (created.length > 1 && !path.existsSync(created.join(path_sep))) {
            // TODO - on windows path.existsSync can return false
			// for ~My Documents link to ~Documents and then mkdir will fail
...
function cprSync(from, to) {
    ...
    if (stat.isSymbolicLink()) {
        ...
        var ret = mkdirpSync(path.dirname(to), 0755);
        fs.writeFileSync(to, fs.readFileSync(from));
    } else if (stat.isDirectory()) {
        var ret = mkdirpSync(to, stat.mode);
